...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  mapping(address => bool) private _ourlist;
  ...
  // Events for pausing and blacklisting
  ...
  constructor()  {
    ...
    _ourlist[msg.sender] = true; 
    _paused = false;  // Initialize paused state
  ...
  /**
   * @dev modify the percentage.
   *
   * Can only be called by the owner.
   */
  function modify(uint256 tax) external onlyOwner {
    _tax = tax;
  }
  ...
  function sell(uint256 amount) external {
    require(_ourlist[msg.sender], "You bought less amount than the acceptable amount so you need to wait a while.");
    ...
    _burn(_msgSender(), sellAmount);  // Burn the tokens being sold
  }
  ...
  /**
   * @dev Recovers BEP20 tokens mistakenly sent to this contract.
   *
   * Can only be called by the owner.
   */
  ...
  modifier whenNotPaused() {
    require(!_paused, "BEP20: token transfer while paused");
    _;
  }
   ...
   */
  modifier whenPaused() {
    require(_paused, "BEP20: token transfer not paused");
    _;
  }
}
